...
the corresponding benchmark tests are executed only once and no metrics are collected when you
run `just test` or `uv run pytest` locally.
...
We also use [bencher.dev](https://bencher.dev/) in the CI for tracking custom metrics like number of nodes or
size in bytes of the hugrs generated by our benchmarks. This is also done for every PR
and main.

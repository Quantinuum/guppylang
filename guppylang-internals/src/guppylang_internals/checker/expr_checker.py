...
class ExprSynthesizer(AstVisitor[tuple[ast.expr, Type]]):
    ...
    def visit_UncheckedMatchPred(
        ...
        if not isinstance(subj_ty, StructType | EnumType | NumericType) and not (
            ...
            raise GuppyError(
                UnsupportedError(node.subject,
                f"Pattern matching on {subj_ty.kind_str()}", True)
            )
...
class PatternChecker(AstVisitor[ast.pattern]):
    ...
    def _check_patterns_args(
        self, patterns: list[ast.pattern], exp_arg_tys: list[Type]
    ) -> list[ast.pattern]:
        checked_patterns = []
        for pattern, exp_ty in zip(patterns, exp_arg_tys, strict=True):
            checked_patterns.append(self.check(pattern, exp_ty))
    ...
    def visit_MatchClass(self, node: ast.MatchClass, exp_ty: Type) -> ast.pattern:
        ...
        match node.cls:
            case ast.Attribute(value=ast.Name(), attr=attr):
                ...
                assert isinstance(node.cls.value, ast.Name)
                class_def = self._get_class_def(node.cls.value, "enum")
            ...
            case ast.Name():
                ...
                class_def = self._get_class_def(node.cls, "struct")
...
def check_num_args(
    ...
    Args:
        exp (int): The expected number of arguments.
        act (int): The actual number of arguments provided.
        node (AstNode): The function call AST node.
        sig (FunctionType | None, optional): The function signature.

...
from guppylang_internals.compiler.builder import DFBuilder, MakeTuple, UnpackTuple
...
def unpack_guppy_object(
    ...
    match obj._ty:
        ...
        case TupleType(element_types=tys):
            unpack = builder.add_op(UnpackTuple(), obj._use_wire(None))
            return tuple(
...
def guppy_object_from_py(
    ...
    match v:
        ...
        case None:
            return GuppyObject(NoneType(), builder.add_op(MakeTuple()))
        case tuple(vs):
            objs = [guppy_object_from_py(v, builder, node, ctx) for v in vs]
            return GuppyObject(
                TupleType([obj._ty for obj in objs]),
                builder.add_op(MakeTuple(), *(obj._use_wire(None) for obj in objs)),
            )
        case GuppyStructObject(_ty=struct_ty, _field_values=values):
            ...
            return GuppyObject(struct_ty, builder.add_op(MakeTuple(), *wires))
...
def update_packed_value(v: Any, obj: "GuppyObject", builder: DFBuilder) -> bool:
    ...
    match v:
        ...
        case tuple(vs):
            ...
            wire_iterator = builder.add_op(UnpackTuple(), obj._use_wire(None)).outputs()
